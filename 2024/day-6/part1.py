...
def get_unique_spots_visited(starting_position, mapped_area):
    unique_spots_visited = set()
    current_row, current_col = starting_position
    unique_spots_visited.add(starting_position)
    ...
    while (next_spot_character is not None):
        if (next_row < 0) or (next_col < 0):
            break
        current_direction = mapped_area[current_row][current_col]
        print(current_direction, current_row, current_col)
        ...
        next_spot_character, next_row, next_col = get_next_spot_character(starting_position, mapped_area)
        unique_spots_visited.add(starting_position)
    #pprint(unique_spots_visited)
    return unique_spots_visited
...
def main():
    ...
    unique_spots_visited = get_unique_spots_visited(starting_position, output)

    for unique_spot_visited in unique_spots_visited:
        row, col = unique_spot_visited
        output[row][col] = "^"
